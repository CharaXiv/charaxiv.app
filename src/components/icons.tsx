import type { FC } from 'hono/jsx'

// All icons use currentColor and have consistent sizing
const iconClass = "w-[1em] h-[1em] align-[-0.125em] shrink-0"

export const IconLayerGroup: FC = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512" fill="currentColor" class={iconClass}>
    <path d="M264.5 5.2c14.9-6.9 32.1-6.9 47 0l218.6 101c8.5 3.9 13.9 12.4 13.9 21.8s-5.4 17.9-13.9 21.8l-218.6 101c-14.9 6.9-32.1 6.9-47 0L45.9 149.8C37.4 145.8 32 137.3 32 128s5.4-17.9 13.9-21.8L264.5 5.2zM476.9 209.6l53.2 24.6c8.5 3.9 13.9 12.4 13.9 21.8s-5.4 17.9-13.9 21.8l-218.6 101c-14.9 6.9-32.1 6.9-47 0L45.9 277.8C37.4 273.8 32 265.3 32 256s5.4-17.9 13.9-21.8l53.2-24.6 152 70.2c23.4 10.8 50.4 10.8 73.8 0l152-70.2zm-152 198.2l152-70.2 53.2 24.6c8.5 3.9 13.9 12.4 13.9 21.8s-5.4 17.9-13.9 21.8l-218.6 101c-14.9 6.9-32.1 6.9-47 0L45.9 405.8C37.4 401.8 32 393.3 32 384s5.4-17.9 13.9-21.8l53.2-24.6 152 70.2c23.4 10.8 50.4 10.8 73.8 0z"></path>
  </svg>
)

export const IconSpinner: FC = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" fill="currentColor" class={`${iconClass} icon-spinner hidden animate-spin`}>
    <path d="M304 48a48 48 0 1 0 -96 0 48 48 0 1 0 96 0zm0 416a48 48 0 1 0 -96 0 48 48 0 1 0 96 0zM48 304a48 48 0 1 0 0-96 48 48 0 1 0 0 96zm464-48a48 48 0 1 0 -96 0 48 48 0 1 0 96 0zM142.9 437A48 48 0 1 0 75 369.1 48 48 0 1 0 142.9 437zm0-294.2A48 48 0 1 0 75 75a48 48 0 1 0 67.9 67.9zM369.1 437A48 48 0 1 0 437 369.1 48 48 0 1 0 369.1 437z"></path>
  </svg>
)

export const IconEye: FC = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512" fill="currentColor" class={iconClass}>
    <path d="M288 32c-80.8 0-145.5 36.8-192.6 80.6C48.6 156 17.3 208 2.5 243.7c-3.3 7.9-3.3 16.7 0 24.6C17.3 304 48.6 356 95.4 399.4C142.5 443.2 207.2 480 288 480s145.5-36.8 192.6-80.6c46.8-43.5 78.1-95.4 93-131.1c3.3-7.9 3.3-16.7 0-24.6c-14.9-35.7-46.2-87.7-93-131.1C433.5 68.8 368.8 32 288 32zM144 256a144 144 0 1 1 288 0 144 144 0 1 1 -288 0zm144-64c0 35.3-28.7 64-64 64c-7.1 0-13.9-1.2-20.3-3.3c-5.5-1.8-11.9 1.6-11.7 7.4c.3 6.9 1.3 13.8 3.2 20.7c13.7 51.2 66.4 81.6 117.6 67.9s81.6-66.4 67.9-117.6c-11.1-41.5-47.8-69.4-88.6-71.1c-5.8-.2-9.2 6.1-7.4 11.7c2.1 6.4 3.3 13.2 3.3 20.3z"></path>
  </svg>
)

export const IconEyeSlash: FC = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512" fill="currentColor" class={iconClass}>
    <path d="M38.8 5.1C28.4-3.1 13.3-1.2 5.1 9.2S-1.2 34.7 9.2 42.9l592 464c10.4 8.2 25.5 6.3 33.7-4.1s6.3-25.5-4.1-33.7L525.6 386.7c39.6-40.6 66.4-86.1 79.9-118.4c3.3-7.9 3.3-16.7 0-24.6c-14.9-35.7-46.2-87.7-93-131.1C465.5 68.8 400.8 32 320 32c-68.2 0-125 26.3-169.3 60.8L38.8 5.1zM223.1 149.5C248.6 126.2 282.7 112 320 112c79.5 0 144 64.5 144 144c0 24.9-6.3 48.3-17.4 68.7L408 294.5c8.4-19.3 10.6-41.4 4.8-63.3c-11.1-41.5-47.8-69.4-88.6-71.1c-5.8-.2-9.2 6.1-7.4 11.7c2.1 6.4 3.3 13.2 3.3 20.3c0 10.2-2.4 19.8-6.6 28.3l-90.3-70.8zM373 389.9c-16.4 6.5-34.3 10.1-53 10.1c-79.5 0-144-64.5-144-144c0-6.9 .5-13.6 1.4-20.2L83.1 161.5C60.3 191.2 44 220.8 34.5 243.7c-3.3 7.9-3.3 16.7 0 24.6c14.9 35.7 46.2 87.7 93 131.1C174.5 443.2 239.2 480 320 480c47.8 0 89.9-12.9 126.2-32.5L373 389.9z"></path>
  </svg>
)

export const IconAddressBook: FC = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" fill="currentColor" class={iconClass}>
    <path d="M96 0C60.7 0 32 28.7 32 64l0 384c0 35.3 28.7 64 64 64l288 0c35.3 0 64-28.7 64-64l0-384c0-35.3-28.7-64-64-64L96 0zM208 288l64 0c44.2 0 80 35.8 80 80c0 8.8-7.2 16-16 16l-192 0c-8.8 0-16-7.2-16-16c0-44.2 35.8-80 80-80zm-32-96a64 64 0 1 1 128 0 64 64 0 1 1 -128 0zM512 80c0-8.8-7.2-16-16-16s-16 7.2-16 16l0 64c0 8.8 7.2 16 16 16s16-7.2 16-16l0-64zM496 192c-8.8 0-16 7.2-16 16l0 64c0 8.8 7.2 16 16 16s16-7.2 16-16l0-64c0-8.8-7.2-16-16-16zm16 144c0-8.8-7.2-16-16-16s-16 7.2-16 16l0 64c0 8.8 7.2 16 16 16s16-7.2 16-16l0-64z"></path>
  </svg>
)

export const IconArrowUpFromBracket: FC = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" fill="currentColor" class={iconClass}>
    <path d="M246.6 9.4c-12.5-12.5-32.8-12.5-45.3 0l-128 128c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192 109.3 192 320c0 17.7 14.3 32 32 32s32-14.3 32-32l0-210.7 73.4 73.4c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3l-128-128zM64 352c0-17.7-14.3-32-32-32s-32 14.3-32 32l0 64c0 53 43 96 96 96l256 0c53 0 96-43 96-96l0-64c0-17.7-14.3-32-32-32s-32 14.3-32 32l0 64c0 17.7-14.3 32-32 32L96 448c-17.7 0-32-14.3-32-32l0-64z"></path>
  </svg>
)

export const IconAngleLeft: FC = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" fill="currentColor" class={iconClass}>
    <path d="M41.4 233.4c-12.5 12.5-12.5 32.8 0 45.3l160 160c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L109.3 256 246.6 118.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0l-160 160z"></path>
  </svg>
)

export const IconAngleRight: FC = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" fill="currentColor" class={iconClass}>
    <path d="M278.6 233.4c12.5 12.5 12.5 32.8 0 45.3l-160 160c-12.5 12.5-32.8 12.5-45.3 0s-12.5-32.8 0-45.3L210.7 256 73.4 118.6c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0l160 160z"></path>
  </svg>
)

export const IconAnglesLeft: FC = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" fill="currentColor" class={iconClass}>
    <path d="M41.4 233.4c-12.5 12.5-12.5 32.8 0 45.3l160 160c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L109.3 256 246.6 118.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0l-160 160zm352-160l-160 160c-12.5 12.5-12.5 32.8 0 45.3l160 160c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L301.3 256 438.6 118.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0z"></path>
  </svg>
)

export const IconAnglesRight: FC = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" fill="currentColor" class={iconClass}>
    <path d="M470.6 278.6c12.5-12.5 12.5-32.8 0-45.3l-160-160c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L402.7 256 265.4 393.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0l160-160zm-352 160l160-160c12.5-12.5 12.5-32.8 0-45.3l-160-160c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L210.7 256 73.4 393.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0z"></path>
  </svg>
)

export const IconSquare: FC = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" fill="currentColor" class={iconClass}>
    <path d="M384 80c8.8 0 16 7.2 16 16l0 320c0 8.8-7.2 16-16 16L64 432c-8.8 0-16-7.2-16-16L48 96c0-8.8 7.2-16 16-16l320 0zM64 32C28.7 32 0 60.7 0 96L0 416c0 35.3 28.7 64 64 64l320 0c35.3 0 64-28.7 64-64l0-320c0-35.3-28.7-64-64-64L64 32z"></path>
  </svg>
)

export const IconSquareCheck: FC = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" fill="currentColor" class={iconClass}>
    <path d="M64 32C28.7 32 0 60.7 0 96L0 416c0 35.3 28.7 64 64 64l320 0c35.3 0 64-28.7 64-64l0-320c0-35.3-28.7-64-64-64L64 32zM337 209L209 337c-9.4 9.4-24.6 9.4-33.9 0l-64-64c-9.4-9.4-9.4-24.6 0-33.9s24.6-9.4 33.9 0l47 47L303 175c9.4-9.4 24.6-9.4 33.9 0s9.4 24.6 0 33.9z"></path>
  </svg>
)

export const IconCaretDown: FC = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" fill="currentColor" class={iconClass}>
    <path d="M137.4 374.6c12.5 12.5 32.8 12.5 45.3 0l128-128c9.2-9.2 11.9-22.9 6.9-34.9s-16.6-19.8-29.6-19.8L32 192c-12.9 0-24.6 7.8-29.6 19.8s-2.2 25.7 6.9 34.9l128 128z"></path>
  </svg>
)

export const IconPlus: FC = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" fill="currentColor" class={iconClass}>
    <path d="M256 80c0-17.7-14.3-32-32-32s-32 14.3-32 32l0 144L48 224c-17.7 0-32 14.3-32 32s14.3 32 32 32l144 0 0 144c0 17.7 14.3 32 32 32s32-14.3 32-32l0-144 144 0c17.7 0 32-14.3 32-32s-14.3-32-32-32l-144 0 0-144z"></path>
  </svg>
)

export const IconTrash: FC = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" fill="currentColor" class={iconClass}>
    <path d="M135.2 17.7L128 32 32 32C14.3 32 0 46.3 0 64S14.3 96 32 96l384 0c17.7 0 32-14.3 32-32s-14.3-32-32-32l-96 0-7.2-14.3C307.4 6.8 296.3 0 284.2 0L163.8 0c-12.1 0-23.2 6.8-28.6 17.7zM416 128L32 128 53.2 467c1.6 25.3 22.6 45 47.9 45l245.8 0c25.3 0 46.3-19.7 47.9-45L416 128z"></path>
  </svg>
)

export const LogoCharaXiv: FC = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 764.31 106.16" fill="currentColor" class="h-4 w-auto hidden sm:inline">
    <g>
      <path d="M54.54,105.14h-16.53C11.57,105.14,0,89.25,0,65.48v-26.19C0,17.93,11.44,1.27,38.78,1.27h14.75c26.95,0,37.76,15.51,37.76,33.56v.38c0,2.42-1.65,3.94-5.59,3.94-4.2,0-5.34-1.27-5.72-3.43v-.51c0-14.11-8.9-22.88-26.44-22.88h-14.75c-17.04,0-27.46,9.66-27.46,26.95v26.19c0,18.94,11.06,28.73,26.7,28.73h16.53c16.27,0,25.68-8.39,25.68-24.54v-.64c.25-2.42,1.27-4.07,5.72-4.07,4.07,0,5.59,1.4,5.59,3.94v.64c0,21.23-11.06,35.6-37,35.6Z"></path>
      <path d="M193.11,105.14h-2.67c-2.54,0-4.45-1.4-4.45-4.2V57.72c0-14.24-8.01-19.83-21.23-19.83h-8.52c-16.15,0-21.74,10.3-21.74,18.18v44.88c0,2.8-1.65,4.2-4.32,4.2h-2.67c-2.8,0-4.45-1.4-4.45-4.2V5.72c0-2.8,1.65-4.2,4.32-4.2h2.67c2.67,0,4.45,1.4,4.45,4.2v30.64c5.08-5.85,12.97-9.54,22.76-9.54h8.65c20.72,0,31.66,9.79,31.66,30v44.12c0,2.8-1.78,4.2-4.45,4.2Z"></path>
      <path d="M279.56,105.14h-27.72c-18.56,0-25.81-8.01-25.81-21.87v-4.96c0-13.1,6.48-21.74,25.55-21.74h31.91c4.2,0,7.75,.76,10.68,1.91v-7.63c0-7.5-3.18-13.6-16.02-13.6h-43.99c-2.92,0-4.07-1.53-4.07-4.45v-1.53c0-2.92,1.14-4.45,4.07-4.45h46.41c17.16,0,25.05,8.77,25.05,24.16v32.29c0,13.86-7.5,21.87-26.06,21.87Zm14.62-29.5c0-7.12-4.2-9.92-12.33-9.92h-28.61c-12.21,0-15.89,3.81-15.89,12.46v4.32c0,8.01,3.05,12.33,15.89,12.33h25.05c12.21,0,15.89-3.43,15.89-12.33v-6.87Z"></path>
      <path d="M390.54,37.76h-21.87c-16.02,0-23.39,8.01-23.39,23.01v40.05c0,2.8-1.91,4.32-4.58,4.32h-2.29c-2.67,0-4.45-1.53-4.45-4.32V61.03c0-23.27,13.09-34.33,33.18-34.33h23.39c2.92,0,4.32,1.65,4.32,4.45v2.03c0,2.8-1.4,4.58-4.32,4.58Z"></path>
      <path d="M473.18,105.14h-27.72c-18.56,0-25.81-8.01-25.81-21.87v-4.96c0-13.1,6.48-21.74,25.55-21.74h31.91c4.2,0,7.75,.76,10.68,1.91v-7.63c0-7.5-3.18-13.6-16.02-13.6h-43.99c-2.92,0-4.07-1.53-4.07-4.45v-1.53c0-2.92,1.14-4.45,4.07-4.45h46.41c17.16,0,25.05,8.77,25.05,24.16v32.29c0,13.86-7.5,21.87-26.06,21.87Zm14.62-29.5c0-7.12-4.2-9.92-12.33-9.92h-28.61c-12.21,0-15.89,3.81-15.89,12.46v4.32c0,8.01,3.05,12.33,15.89,12.33h25.05c12.21,0,15.89-3.43,15.89-12.33v-6.87Z"></path>
      <path d="M603.61,106.16c-2.29,0-3.69-.76-4.96-2.54l-31.15-41.83-31.28,41.83c-1.14,1.65-2.67,2.54-4.96,2.54-3.81,0-6.48-1.27-6.48-4.2,0-.89,.38-2.03,1.14-3.18l34.58-46.4-33.44-45.13c-.89-1.27-1.4-2.29-1.4-3.18,0-2.67,3.31-4.07,6.87-4.07,2.42,0,3.81,.89,4.83,2.54l30.13,40.43,30-40.43c1.14-1.53,2.67-2.54,4.7-2.54,3.56,0,6.99,1.4,6.99,4.07,0,.89-.38,1.91-1.27,3.18l-33.44,45.13,34.45,46.4c.89,1.14,1.27,2.29,1.27,3.31,0,2.8-2.8,4.07-6.61,4.07Z"></path>
      <path d="M648.23,17.29h-2.42c-2.8,0-4.45-1.53-4.45-4.32V5.85c0-2.92,1.65-4.32,4.45-4.32h2.42c2.67,0,4.58,1.4,4.58,4.32v7.12c0,2.8-1.91,4.32-4.58,4.32Zm0,87.72h-2.42c-2.8,0-4.45-1.4-4.45-4.32V32.67c0-2.8,1.78-4.32,4.45-4.32h2.42c2.67,0,4.58,1.53,4.58,4.32V100.69c0,2.92-1.91,4.32-4.58,4.32Z"></path>
      <path d="M763.92,31.78l-15.64,52.76c-3.81,12.97-7.12,20.72-21.23,20.72h-5.09c-12.21,0-17.29-5.72-21.61-20.47l-15.38-52.89c-.25-.76-.38-1.4-.38-2.03,0-2.03,1.4-3.18,3.56-3.18h2.16c2.92,0,4.83,1.14,5.59,3.94l14.49,50.09c2.16,7.5,4.07,13.22,11.95,13.22h4.07c8.26,0,10.04-5.85,12.33-13.6l14.37-49.71c.89-2.92,2.54-3.94,5.47-3.94h2.16c2.42,0,3.56,1.14,3.56,3.05,0,.64-.13,1.27-.38,2.03Z"></path>
    </g>
  </svg>
)

export const IconChevronLeft: FC = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" fill="currentColor" class={iconClass}>
    <path d="M9.4 233.4c-12.5 12.5-12.5 32.8 0 45.3l192 192c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L77.3 256 246.6 86.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0l-192 192z"></path>
  </svg>
)

export const IconChevronRight: FC = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" fill="currentColor" class={iconClass}>
    <path d="M310.6 233.4c12.5 12.5 12.5 32.8 0 45.3l-192 192c-12.5 12.5-32.8 12.5-45.3 0s-12.5-32.8 0-45.3L242.7 256 73.4 86.6c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0l192 192z"></path>
  </svg>
)

export const IconImage: FC<{ class?: string }> = ({ class: className }) => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" fill="currentColor" class={className || iconClass}>
    <path d="M0 96C0 60.7 28.7 32 64 32l384 0c35.3 0 64 28.7 64 64l0 320c0 35.3-28.7 64-64 64L64 480c-35.3 0-64-28.7-64-64L0 96zM323.8 202.5c-4.5-6.6-11.9-10.5-19.8-10.5s-15.4 3.9-19.8 10.5l-87 127.6L170.7 297c-4.6-5.7-11.5-9-18.7-9s-14.2 3.3-18.7 9l-64 80c-5.8 7.2-6.9 17.1-2.9 25.4s12.4 13.6 21.6 13.6l96 0 32 0 208 0c8.9 0 17.1-4.9 21.2-12.8s3.6-17.4-1.4-24.7l-120-176zM112 192a48 48 0 1 0 0-96 48 48 0 1 0 0 96z"></path>
  </svg>
)

export const IconDownload: FC = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" fill="currentColor" class={iconClass}>
    <path d="M288 32c0-17.7-14.3-32-32-32s-32 14.3-32 32l0 242.7-73.4-73.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3l128 128c12.5 12.5 32.8 12.5 45.3 0l128-128c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L288 274.7 288 32zM64 352c-35.3 0-64 28.7-64 64l0 32c0 35.3 28.7 64 64 64l384 0c35.3 0 64-28.7 64-64l0-32c0-35.3-28.7-64-64-64l-101.5 0-45.3 45.3c-25 25-65.5 25-90.5 0L165.5 352 64 352zm368 56a24 24 0 1 1 0 48 24 24 0 1 1 0-48z"></path>
  </svg>
)

export const IconThumbtack: FC<{ class?: string }> = ({ class: className }) => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512" fill="currentColor" class={className || iconClass}>
    <path d="M32 32C32 14.3 46.3 0 64 0L320 0c17.7 0 32 14.3 32 32s-14.3 32-32 32l-29.5 0 11.4 148.2c36.7 19.9 65.7 53.2 79.5 94.7l1 3c3.3 9.8 1.6 20.5-4.4 28.8s-15.7 13.3-26 13.3L192 352l0 128c0 17.7-14.3 32-32 32s-32-14.3-32-32l0-128L32 352c-10.3 0-19.9-4.9-26-13.3s-7.7-19.1-4.4-28.8l1-3c13.8-41.5 42.8-74.8 79.5-94.7L93.5 64 64 64C46.3 64 32 49.7 32 32z"></path>
  </svg>
)

// X mark for close button
export const IconClose: FC<{ class?: string }> = ({ class: className }) => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512" fill="currentColor" class={className || iconClass}>
    <path d="M342.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L192 210.7 86.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L146.7 256 41.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192 301.3 297.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L237.3 256 342.6 150.6z"></path>
  </svg>
)

// Grid icon for gallery view
export const IconGrid: FC<{ class?: string }> = ({ class: className }) => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" fill="currentColor" class={className || iconClass}>
    <path d="M0 72C0 49.9 17.9 32 40 32l48 0c22.1 0 40 17.9 40 40l0 48c0 22.1-17.9 40-40 40l-48 0c-22.1 0-40-17.9-40-40L0 72zM0 232c0-22.1 17.9-40 40-40l48 0c22.1 0 40 17.9 40 40l0 48c0 22.1-17.9 40-40 40l-48 0c-22.1 0-40-17.9-40-40l0-48zM128 392l0 48c0 22.1-17.9 40-40 40l-48 0c-22.1 0-40-17.9-40-40l0-48c0-22.1 17.9-40 40-40l48 0c22.1 0 40 17.9 40 40zM160 72c0-22.1 17.9-40 40-40l48 0c22.1 0 40 17.9 40 40l0 48c0 22.1-17.9 40-40 40l-48 0c-22.1 0-40-17.9-40-40l0-48zM288 232l0 48c0 22.1-17.9 40-40 40l-48 0c-22.1 0-40-17.9-40-40l0-48c0-22.1 17.9-40 40-40l48 0c22.1 0 40 17.9 40 40zM160 392c0-22.1 17.9-40 40-40l48 0c22.1 0 40 17.9 40 40l0 48c0 22.1-17.9 40-40 40l-48 0c-22.1 0-40-17.9-40-40l0-48zM360 32l48 0c22.1 0 40 17.9 40 40l0 48c0 22.1-17.9 40-40 40l-48 0c-22.1 0-40-17.9-40-40l0-48c0-22.1 17.9-40 40-40zM448 232l0 48c0 22.1-17.9 40-40 40l-48 0c-22.1 0-40-17.9-40-40l0-48c0-22.1 17.9-40 40-40l48 0c22.1 0 40 17.9 40 40zM320 392c0-22.1 17.9-40 40-40l48 0c22.1 0 40 17.9 40 40l0 48c0 22.1-17.9 40-40 40l-48 0c-22.1 0-40-17.9-40-40l0-48z"></path>
  </svg>
)
